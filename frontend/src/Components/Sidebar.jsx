...
const menu = [
  {
    name: "Dashboard",
    icon: <HomeIcon className="w-6 h-6 d_icon" />,
    link: "",
  },
  {
    name: "Category",
  ...
  {
    name: "Privacy policy",
    icon: <ShieldCheckIcon className="w-6 h-6 d_icon" />,
    link: "/privacy_policy",
  },
];
...
const Sidebar = ({ open, setopen }) => {
  ...
  return (
    <aside
      className={`fixed z-40 top-0 left-0 h-screen d_sidebar shadow-lg transition-transform duration-300
...
`}
    >
      <div className="relative flex items-center py-[20px] border-b d_border pl-4 pr-5 flex-shrink-0">
        <span
          className={`text-2xl font-bold tracking-widest d_logo transition-all duration-300 ${
        ...
        <button
          className="md:hidden absolute top-1/2 right-4 -translate-y-1/2 flex items-center justify-center w-9 h-9 rounded-full transition shadow-lg bg-white"
            ...
            <li key={item.name} className="relative group">
              {item.submenu ? (
                <>
                  <button
                    type="button"
                    className={`flex items-center w-full px-6 py-3 rounded-lg transition-all d_menu_item font-medium hover:bg-d_hover hover:shadow-lg ${
                    ...
                    >
                      {item.submenu.map((sub) => (
                        <li key={sub.name}>
                          <Link
                            to={sub.link}
                            className={`flex items-center gap-2 px-6 py-2 hover:bg-[#254D70]/20 hover:text-[#254D70] transition-colors duration-150 ${
                              location.pathname === sub.link
                                ? "bg-[#254D70]/10 text-[#254D70] font-semibold"
                                : "text-gray-700"
                            }`}
                            onClick={() => setopen(false)}
                          >
                ...
                <Link
                  to={item.link}
                  className={`flex items-center px-6 py-3 rounded-lg transition-all d_menu_item font-medium hover:bg-d_hover hover:shadow-lg ${
                    location.pathname === item.link ? "d_active" : "text-dark"
                  } ${effectiveCollapsed ? "justify-center px-2" : ""}`}
                  onClick={() => setopen(false)}
                  tabIndex={0}
